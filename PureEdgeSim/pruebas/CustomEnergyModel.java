...
public class CustomEnergyModel extends EnergyModel {
	...
	public void updateCpuEnergyConsumption(double cpuUtilization) {
		...
		// if the CPU operates at 100% it will consumes the amount of maxActiveConsumption.
		// the energy consumed each second will be between [idleConsumption, maxActiveConsumption],
		// for any cpu usage [0-->100%].
		// so 0% will give the idle consumption,
		// and 100% will give the max consumption (we have to avoid counting the idle consumption twice).
		...
		// the update interval can be < 1 second (e.g., 0.1) or > 1 second (e.g., 2 seconds).
		// so we need to multiply the results by the update interval in order to get the correct measuring
		...
		// however the consumed energy that we calculated is for 1 seconds,
		// we need to multiply it by the interval in order to get the correct consumption
	...
	public void updatewirelessEnergyConsumption(FileTransferProgress file, DataCenter device1, DataCenter device2,
		...
		// for a more realistic model, please check the default PureEdgeSim Energy Model
		// (DefaultEnergyModel)
	}
