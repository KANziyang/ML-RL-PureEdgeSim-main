...
public class FiveAgentDecisionSupport {
	public static final int AGENT_COUNT = 5;
	public static final int LOCAL_OBS_SIZE = 14;
	public static final int GLOBAL_EXTRA_SIZE = 6;
	public static final int GLOBAL_STATE_SIZE = AGENT_COUNT * LOCAL_OBS_SIZE + GLOBAL_EXTRA_SIZE;
	public static final int PRIORITY_ACTIONS = 5;
	public static final int[] PRIORITY_BIN_MAP = { 0, 2, 5, 8, 10 };
	public static final String[] EDGE_CLOUD_ARCH = { "Cloud", "Edge" };
	...
	private final List<DataCenter> agentNodes = new ArrayList<DataCenter>();
	...
	public static class VmCandidate {
		...
		public VmCandidate(int vmIndex, double estimatedFinishTime, double estimatedFinishOverDeadline,
				boolean networkAdmissible, SimulationParameters.TYPES nodeType) {
	...
	public static class StateBundle {
		public final double[][] localObs;
		public final double[] globalState;
		public final int[] actionMask;
		public final VmCandidate[] candidates;

		public StateBundle(double[][] localObs, double[] globalState, int[] actionMask, VmCandidate[] candidates) {
	...
	public static class DecisionMeta {
		...
		public final int selectedAgent;
		public final int selectedVm;
		public final int selectedPriorityBin;
		public final long stepId;

		public DecisionMeta(double[][] obs, double[] state, int[] actionMask, int destAction, int priorityAction,
	...
	public static class DecisionTelemetrySnapshot {
		...
		public DecisionTelemetrySnapshot(int windowDecisionCount, int[] destWindowCounts, int[] priorityWindowCounts,
		...
		public static DecisionTelemetrySnapshot empty() {
			return new DecisionTelemetrySnapshot(0, new int[AGENT_COUNT], new int[PRIORITY_ACTIONS], -1, -1);
		}
	}

	public static class DecisionTelemetryTracker {
		private final ArrayDeque<Integer> destWindow = new ArrayDeque<Integer>();
		private final ArrayDeque<Integer> priorityWindow = new ArrayDeque<Integer>();
		...
		public synchronized void update(int selectedAgent, int selectedPriorityBin) {
		...
		public synchronized DecisionTelemetrySnapshot snapshot() {
			return new DecisionTelemetrySnapshot(destWindow.size(), destWindowCounts, priorityWindowCounts,
					lastDestAction, lastPriorityBin);
		}
	}

	public FiveAgentDecisionSupport(SimulationManager simulationManager, List<List<Integer>> orchestrationHistory,
			List<Vm> vmList) {
	...
	public StateBundle buildState(Task task, String[] architecture) {
	...
	public DecisionMeta createDecisionMeta(StateBundle state, int destAction, int priorityAction, int selectedAgent,
			int selectedVm, int selectedPriorityBin, long stepId) {
		return new DecisionMeta(state.localObs, state.globalState, state.actionMask, destAction, priorityAction,
				selectedAgent, selectedVm, selectedPriorityBin, stepId);
	}

	public void applyPriorityDecision(Task task, int selectedVm, int priorityBin) {
	...
	public int sanitizePriorityAction(int value) {
		return clampInt(value, 0, PRIORITY_ACTIONS - 1);
	}

	public int resolveDestination(int requestedDest, int[] actionMask, VmCandidate[] candidates) {
	...
	public int priorityActionToBin(int priorityAction) {
		return PRIORITY_BIN_MAP[sanitizePriorityAction(priorityAction)];
	}

	public int priorityActionForBin(int priorityBin) {
	...
	public static int priorityIndexForBin(int priorityBin) {
		for (int i = 0; i < PRIORITY_BIN_MAP.length; i++) {
			if (PRIORITY_BIN_MAP[i] == priorityBin) {
				return i;
			}
	...
	private void initializeAgentMapping() {
		List<DataCenter> edgeNodes = new ArrayList<DataCenter>();
		List<DataCenter> cloudNodes = new ArrayList<DataCenter>();
		...
		if (edgeNodes.size() != 4 || cloudNodes.size() != 1) {
			throw new IllegalStateException(
					"FiveAgentDecisionSupport requires exactly 4 EDGE_DATACENTER and 1 CLOUD, found edge="
	...
	private VmCandidate findBestVmForAgent(Task task, int agentIndex, String[] architecture) {
		...
		return new VmCandidate(bestVm, bestFinish, bestRatio, networkAdmissible, dc.getType());
	}

	private VmCandidate invalidCandidate() {
		return new VmCandidate(-1, Double.MAX_VALUE, 2.0, false, SimulationParameters.TYPES.CLOUD);
	}
	...
	private double estimateVmFinishTime(Task task, int vmIndex) {
		...
		double running = Math.max(1.0,
				orchestrationHistory.get(vmIndex).size()
						- vm.getCloudletScheduler().getCloudletFinishedList().size() + 1.0);
