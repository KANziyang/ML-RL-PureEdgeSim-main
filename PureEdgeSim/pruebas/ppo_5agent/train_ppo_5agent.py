...
GAMMA = float(os.getenv("PUREEDGESIM_PPO5_GAMMA", "0.99"))
GAE_LAMBDA = float(os.getenv("PUREEDGESIM_PPO5_GAE_LAMBDA", "0.95"))
CLIP_EPS = float(os.getenv("PUREEDGESIM_PPO5_CLIP", "0.1"))
ACTOR_LR = float(os.getenv("PUREEDGESIM_PPO5_ACTOR_LR", "3e-4"))
CRITIC_LR = float(os.getenv("PUREEDGESIM_PPO5_CRITIC_LR", "3e-4"))
ENTROPY_COEF_START = float(os.getenv("PUREEDGESIM_PPO5_ENTROPY_START", "0.02"))
ENTROPY_COEF_END = float(os.getenv("PUREEDGESIM_PPO5_ENTROPY_END", "0.002"))
VALUE_COEF = float(os.getenv("PUREEDGESIM_PPO5_VALUE_COEF", "0.5"))
MAX_GRAD_NORM = float(os.getenv("PUREEDGESIM_PPO5_MAX_GRAD_NORM", "0.5"))
PPO_EPOCHS = int(os.getenv("PUREEDGESIM_PPO5_EPOCHS", "4"))
MINIBATCH = int(os.getenv("PUREEDGESIM_PPO5_MINIBATCH", "1024"))
EPISODES_PER_UPDATE = int(os.getenv("PUREEDGESIM_PPO5_EPISODES_PER_UPDATE", "4"))
            ...
            if msg_type == "marl_obs":
                ...
                if transition_count % config.progress_log_interval == 0:
                    print(
                        ...
                        f"steps={transition_count} reward_so_far={episode_reward:.4f}",
                        flush=True,
                    )
...
def save_checkpoint(
    ...
    if not latest_only:
        versioned_path = resolve_model_path(config, f"ppo5agent_pureedgesim_ep{episode}_{timestamp}.pt")
...
def main() -> None:
    ...
    try:
        ...
        for episode in range(1, config.episodes + 1):
            ...
            print(
                f"episode={episode}/{config.episodes} "
                f"steps={len(episode_buffer)} reward={episode_reward:.4f} "
