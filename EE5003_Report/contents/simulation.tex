...
The empirical study is organized to answer the three research questions introduced in \Secref{sec:introduction}. The standard-setting MAPPO training run and the heuristic-baseline comparison address whether joint on-policy RL is effective in the studied environment. The controlled NoEmbedding and FixedPRB ablations isolate which parts of the MAPPO design matter most, especially whether adaptive PRB control is the dominant contributor. The MAPPO--PPO comparison and the device-count study then clarify, respectively, the specific value of explicit multi-agent conditioning and the operating range over which the learned policy remains effective.
