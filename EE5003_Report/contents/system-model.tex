...
5.0 - 1.0\widetilde{L}_t - 2.0\widetilde{E}_t - 1.5\widetilde{B}_t - P_t^{\mathrm{fb}}, & \text{if the task succeeds},
\end{cases}
\label{eq:implemented-reward}
\end{align}
where \(P_t^{\mathrm{fb}} = 1.5\) if the requested destination is replaced by a feasible fallback and \(0\) otherwise. This reward is a shaped negative version of the cost in \Eqref{eq:policy-objective}. It prioritizes deadline satisfaction first, then penalizes excessive energy consumption, wasteful PRB reservation, unnecessary latency, and policy choices that require environment-side fallback correction.
