...
Each device consumes CPU energy according to a linear utilization model. Let \(P_{\mathrm{idle}}\) and \(P_{\mathrm{max}}\) denote the idle and maximum active power consumption of the device (in Wh per hour), respectively. When a task begins execution, the energy model updates the host device's CPU energy by
\begin{align}
\Delta E_{\mathrm{cpu}} = \frac{P_{\mathrm{idle}} + (P_{\mathrm{max}} - P_{\mathrm{idle}}) \cdot u}{3600} \cdot \Delta t,
\label{eq:energy-cpu}
\end{align}
where \(u = L / M_{\mathrm{total}}\) is the instantaneous CPU utilization contributed by the task (\(L\) is the task length in MI, \(M_{\mathrm{total}}\) is the total MIPS of the data center), and \(\Delta t\) is the update interval in seconds. The total device energy is the sum of the wireless and CPU components:
\begin{align}
