...
The 13-dimensional agent feature vector encodes the local device condition, including CPU utilization, queue pressure, battery status, mobility-related information, position, and task descriptors. The destination matrix contains one 11-dimensional feature vector for each candidate destination. The candidates are ordered consistently as local execution first, then edge data centers sorted by data-center identifier, and finally cloud destinations. Each destination feature summarizes its type, distance, load, queue state, estimated completion time, and current availability.
...
Each candidate destination feature vector \(d_{i,j}\) is encoded separately:
\begin{align}
z_{i,j} = f_{\theta}^{\mathrm{dest}}(d_{i,j}),
\end{align}
again using a two-layer MLP with \(\tanh\) activations. Destination logits are then computed through an interaction between the agent encoding and every destination encoding,
\begin{align}
\ell_{i,j}^{\mathrm{dest}} = h_i^\top z_{i,j},
\end{align}
...
The PRB head is conditioned on the selected destination. In the implementation, the PRB logits are produced from the joint representation of the agent encoding and the chosen destination encoding, which allows the bandwidth decision to depend on where the task will be executed:
\begin{align}
\pi_{\theta}^{\mathrm{prb}}(k \mid o_i, a_i^{\mathrm{dest}}) =
\mathrm{Softmax}\!\left(g_{\theta}\left([h_i ; z_{i,a_i^{\mathrm{dest}}}]\right)\right).
\end{align}
This coupling is important because the desirable PRB level differs substantially between nearby edge nodes and the farther cloud tier.
...
The PPO baseline shares the same destination encoder, PRB head, and overall dual-head logic, but removes the learnable agent embedding. Its actor receives only the raw 13-dimensional device observation. As a result, PPO must represent all device roles through a single shared feature space, whereas MAPPO can condition each policy evaluation on an explicit agent identity. This architectural difference is the main controlled distinction between the two RL methods.
...
The implementation uses generalized advantage estimation in the shared replay buffer:
\begin{align}
\delta_t &= r_t + \gamma V_{\phi}(s_{t+1}) - V_{\phi}(s_t), \\
\hat{A}_t &= \sum_{l=0}^{T-t-1} (\gamma \lambda)^l \delta_{t+l},
\end{align}
with \(\gamma = 0.99\) and \(\lambda = 0.95\). Although the environment is event-driven and many rewards are dominated by immediate task outcomes, the GAE formulation still improves update smoothness across long episodes.
